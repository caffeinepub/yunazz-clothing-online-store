...
import { useCreateCheckoutSession, usePlaceOrder, useGetCallerUserProfile } from '../hooks/useQueries';
import { toast } from 'sonner';
import { PaymentMethod, OrderStatus } from '../backend';
...
export default function CheckoutDialog({ open, onOpenChange, product, selectedSize }: CheckoutDialogProps) {
  const { identity } = useInternetIdentity();
  const { data: userProfile } = useGetCallerUserProfile();
  const [paymentMethod, setPaymentMethod] = useState<'upi' | 'card' | 'cashOnDelivery'>('cashOnDelivery');
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-6">
              ...
              <div className="flex items-center space-x-3 border rounded-lg p-4 cursor-pointer hover:bg-muted/50 transition-colors">
                ...
                <Label htmlFor="upi" className="flex-1 cursor-pointer flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Pay using UPI apps like Google Pay, PhonePe, Paytm</p>
                  </div>
                </Label>
              </div>
          ...
          <p className="text-xs text-center text-muted-foreground">
            {paymentMethod === 'card' && 'You will be redirected to Stripe for secure payment'}
            {paymentMethod === 'upi' && 'Complete payment using your preferred UPI app'}
            {paymentMethod === 'cashOnDelivery' && 'Pay in cash when your order is delivered'}
          </p>
        </div>
      </DialogContent>
    </Dialog>
