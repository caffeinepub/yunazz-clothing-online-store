                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      {new Date(Number(order.timestamp) / 1000000).toLocaleDateString('en-US', {
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric',
                      })}
              ...
              <CardContent>
                <div className="space-y-4">
                    ...
                    {order.products.map((product, index) => (
                      <div key={index} className="flex items-center gap-4 py-2 border-b last:border-0">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-muted-foreground">₹{Number(product.price)}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  <div className="flex justify-between items-center pt-4 border-t">
                    ...
                    <span className="text-xl font-bold text-primary">₹{Number(order.totalAmount)}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
